...
def plot_agg_total_demand(results: ModelResults, save_img=False):
    ...
    ax.plot_epsilon(params.timestamps, total_demand, color='orange', linewidth=1, label='Total Demand')

    # Plot peaks
    base_load_peak = max(house_load)
    total_load_peak = max(total_demand)

    ax.plot_epsilon(params.timestamps, [base_load_peak for _ in params.timestamps], color='skyblue', linewidth=1, label='Base load peak')
    ax.plot_epsilon(params.timestamps, [total_load_peak for _ in params.timestamps], color='orange', linewidth=1, label='Total load peak')
